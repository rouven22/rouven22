<template>
    <div class="my-6">
            ...
            <div>
                <p class="pl-20 font-bold text-2xl">WELCOME TO HOME PAGE</p>
            </div>
            </div>
             ...
             <table id="table_1" class="w-full items-center justify-center text-sm text-left text-gray-50 dark:text-grey-50">
                <thead class="pr-8 flex text-white w-full uppercase bg-red-900 dark:bg-red-900 dark:text-white">
                    <tr class="pr-8  flex w-full mb-4"> 
                         <th class=" p-4 w-1/4 text-left">  <input v-model="selectAll" type="checkbox" @click="select"></th>
                        ...
                        <th  class="p-4 w-1/4 text-left"> <button  type="button" class="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase hover:bg-blue-700 focus:bg-blue-700 focus:outline-none focus:ring-0 active:bg-blue-800 transition duration-150 ease-in-out">
                                SUBMIT
                                </button> </th> 
                    </tr>
                </thead>
                <tbody class=" flex flex-col overflow-y-scroll " style="height: 50vh;">
                    <tr  v-for="applicant in filteredApplicants" :key="applicant.id" class="pr-4 flex w-full bg-grey-900 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-500 dark:hover:bg-gray-600">
                       <td class="p-4 w-1/4">  <input v-model="selected" type="checkbox" :value="applicant.id"></td>
                        ...
                        <td class="inline-flex">
                           <nuxt-link :to="`/dean/${applicant.id}`">
                            <button  type="button" class="inline-block px-6 py-2.5 bg-green-600 text-white font-medium text-xs leading-tight uppercase hover:bg-green-700 focus:bg-green-700 focus:outline-none focus:ring-0 active:bg-green-800 transition duration-150 ease-in-out" >
                                 ...
                                 <nuxt-link :to="`/dean/approval/${applicant.id}`">
     ...
     select() {
			...
			if (!this.selectAll) {
				for (const applicant in this.applicants) {
					this.selected.push(this.applicants[applicant].id);
				}
			}
		}
	}
